...
class LeanAdjointSolverFlow:
    ...
    def step(self, adj, x_t, t, alpha, alpha_dot, dt, upper_edge_mask):
        adj_t = adj.detach() # detach to avoid gradients


        with torch.enable_grad():
            # turn on autograd for the graph
            x_t_grad = self.model.adjoint(x_t)
            x_t_grad.state = x_t_grad.state.detach().requires_grad_(True)

            v_pred = self.model.velocity_field(x_t_grad, t)

            eps_pred = 2 * v_pred - alpha_dot[0]/(alpha[0]+dt) * x_t_grad

            g_term = (adj_t * eps_pred).sum()
            v = torch.autograd.grad(g_term, x_t_grad.state, retain_graph=False)[0]
        assert v.shape == x_t.shape

        adj_tmh = adj_t + dt * v
        if self.grad_f_k_fn is not None:
            grad_list = self.grad_f_k_fn(x_t, t + dt) # returns list
        ...
        #     print(f"Loss is NaN for t = {t}")
        #     raise Exception(f"Loss is NaN for t = {t}")
    ...
    def solve(self, trajectories, ts):
        ...
        x_1 = trajectories[0] # points at t=1  
        minus_adj = self.grad_reward_fn(x_1) # returns a list of tensors (representing the adjoint) # TODO change here as well
        # print("minus_adj", minus_adj)
        # print("minus_adj len", len(minus_adj))
        # print("minus_adj[0].shape", minus_adj[0].shape)
        ...
        trajs_adj = []
        traj_v_pred = []
        upper_edge_mask = x_1.edata['ue_mask'] # (edges, 1)
        ...
        for i in range(1, T):
            ...
            adj, v_pred = self.step(adj=adj, x_t=x_t, t=t, alpha=alpha, alpha_dot=alpha_dot, dt=dt, upper_edge_mask=upper_edge_mask)
        ...
        res = {
                ...
                'traj_graph': trajectories[1:], # list of dgl graphs (T, )
...
class AMDataset(Dataset):
    def __init__(self, solver_info):
        ...
        self.traj_x = solver_info['traj_x'] #  (T,)
        self.traj_adj = solver_info['traj_adj'] # (T, data_shape)
        self.traj_v_base = solver_info['traj_v_pred'] # (T, data_shape)

        self.T = self.t.size(0) # T = number of time steps
        self.bs = 1 # TODO can we remove?
    ...
    def __getitem__(self, idx):
        return {
            ...
            'row_mask': self.row_mask, # TODO remove and change adjoint update with operations
        }
...
class AdjointMatchingTrainer:
    def __init__(self,
        ...
        # Config
        self.config = config
        self.sampling_config = config.sampling # TODO plug into Sampler
        self.device = device or torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        ...
        self.sampler = EulerMaruyamaSampler(self.base_model, sampling_config.data_shape) if sampler is None else sampler
    ...
    def generate_dataset(self, n_atoms=None):
        ...
        for i in range(iterations):
            with torch.no_grad():
                while True:
                    trajectories, ts = self.sampler.sample_trajectories()
                    sigmas = self.fine_model.memoryless_sigma_t(ts)
    ...
    def train_step(self, sample):
        ...
        traj_x = [g.to(self.device) for g in sample['traj_x']]
        ...
        for idx in idxs:
            ...
            v_fine_t = self.model.velocity_field(x_base_t, t)
    ...
    def finetune(self, dataset, steps=None, debug=False):
        ...
        del dataset
        return losses if debug else sum(losses) / N