...
def main() -> None:
    ...
    parser.add_argument("--max-iterations", type=int, default=1000, help="Points per axis.")
    parser.add_argument(
        "--out-dir",
        type=str,
        default="./data",
    ...
    for i in range(max_iterations):
        ...
        hypervolume_approximations.append(torch.logsumexp(torch.tensor(s_lambda_approximations), dim=0))
    ...
    hypervolume_approximations = hypervolume_approximations.cumsum(axis=0) / np.arange(1, max_iterations + 1)
    running_means *= c_k
    hypervolume_approximations *= c_k
    
    plt.figure(figsize=(8, 6))
    plt.plot(running_means, label="Running Mean of Hypervolume Values")
    plt.plot(hypervolume_approximations, label="Running Mean of Hypervolume Approximations", linestyle='--')
